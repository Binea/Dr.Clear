...
namespace DentalMedical.SQLServerDAL
{
	...
	/// 数据访问类:DCImage
	/// </summary>
	public partial class DCImage:IDCImage
	{
		public DCImage()
		{}
		#region  BasicMethod
		...
		public bool Exists(Guid Id)
		{
			...
			strSql.Append("select count(1) from DCImage");
		...
		/// 增加一条数据
		/// </summary>
		public bool Add(DentalMedical.Model.DCImage model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into DCImage(");
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(DentalMedical.Model.DCImage model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update DCImage set ");
		...
		public bool Delete(Guid Id)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from DCImage ");
			strSql.Append(" where Id=@Id ");
			SqlParameter[] parameters = {
					new SqlParameter("@Id", SqlDbType.UniqueIdentifier,16)			};
			parameters[0].Value = Id;
		...
		/// 得到一个对象实体
		/// </summary>
		public DentalMedical.Model.DCImage GetModel(Guid Id)
		{
			
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 Id,EMR_Code,imagePath,orderId,createDate,createId,modifyDate,modifyId,flag from DCImage ");
			strSql.Append(" where Id=@Id ");
			SqlParameter[] parameters = {
					new SqlParameter("@Id", SqlDbType.UniqueIdentifier,16)			};
			parameters[0].Value = Id;
		...
		/// 得到一个对象实体
		/// </summary>
		public DentalMedical.Model.DCImage DataRowToModel(DataRow row)
		{
			DentalMedical.Model.DCImage model=new DentalMedical.Model.DCImage();
			if (row != null)
			{
		...
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
			...
			strSql.Append(")AS Row, T.*  from DCImage T ");
        ...
        /// <returns></returns>
        public List<DentalMedical.Model.DCImage> get_DCImage_List(string emrCode)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("SELECT * FROM dbo.DCImage WHERE EMR_Code=@EMR_Code ORDER BY orderId");
            SqlParameter[] parameters = {
					new SqlParameter("@EMR_Code", SqlDbType.VarChar)			};
            parameters[0].Value = emrCode;

           
            List<DentalMedical.Model.DCImage> List = new List<Model.DCImage>();
            DataSet ds = DbHelperSQL.Query(strSql.ToString(), parameters);
            if (ds.Tables[0].Rows.Count > 0)
            {
               ...
               List= DentalMedical.Common.DataTableList.ToList<DentalMedical.Model.DCImage>(ds.Tables[0]);
               return List;
            }
            else
            {
        ...
        /// 增加一条数据
        /// </summary>
        public Hashtable Add_hash(DentalMedical.Model.DCImage model)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("insert into DCImage(");
            strSql.Append("Id,EMR_Code,imagePath,orderId,createDate,createId,flag)");
