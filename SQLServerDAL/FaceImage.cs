...
namespace DentalMedical.SQLServerDAL
{
	...
	/// 数据访问类:FaceImage
	/// </summary>
	public partial class FaceImage:IFaceImage
	{
		public FaceImage()
		{}
		#region  BasicMethod
		...
		public bool Exists(Guid Id)
		{
			...
			strSql.Append("select count(1) from FaceImage");
		...
		/// 增加一条数据
		/// </summary>
		public bool Add(DentalMedical.Model.FaceImage model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into FaceImage(");
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(DentalMedical.Model.FaceImage model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update FaceImage set ");
		...
		public bool Delete(Guid Id)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from FaceImage ");
			strSql.Append(" where Id=@Id ");
			SqlParameter[] parameters = {
					new SqlParameter("@Id", SqlDbType.UniqueIdentifier,16)			};
			parameters[0].Value = Id;
		...
		/// 得到一个对象实体
		/// </summary>
		public DentalMedical.Model.FaceImage GetModel(Guid Id)
		{
			
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 Id,EMR_Code,imagePath,orderId,createDate,createId,modifyDate,modifyId,flag from FaceImage ");
			strSql.Append(" where Id=@Id ");
			SqlParameter[] parameters = {
					new SqlParameter("@Id", SqlDbType.UniqueIdentifier,16)			};
			parameters[0].Value = Id;
		...
		/// 得到一个对象实体
		/// </summary>
		public DentalMedical.Model.FaceImage DataRowToModel(DataRow row)
		{
			DentalMedical.Model.FaceImage model=new DentalMedical.Model.FaceImage();
			if (row != null)
			{
		...
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
			...
			strSql.Append(")AS Row, T.*  from FaceImage T ");
        ...
        /// <returns></returns>
        public List<DentalMedical.Model.FaceImage> get_FaceImage_List(string emrCode)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("SELECT * FROM dbo.FaceImage WHERE EMR_Code=@EMR_Code ORDER BY orderId");
            SqlParameter[] parameters = {
					new SqlParameter("@EMR_Code", SqlDbType.VarChar)			};
            parameters[0].Value = emrCode;

            List<DentalMedical.Model.FaceImage> List = new List<Model.FaceImage>();
            DataSet ds = DbHelperSQL.Query(strSql.ToString(), parameters);
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                List = DentalMedical.Common.DataTableList.ToList<DentalMedical.Model.FaceImage>(ds.Tables[0]);
                return List;
            }
            else
            {
        ...
        /// 增加一条数据
        /// </summary>
        public Hashtable Add_hash(DentalMedical.Model.FaceImage model)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("insert into FaceImage(");
            strSql.Append("Id,EMR_Code,imagePath,orderId,createDate,createId,flag)");
